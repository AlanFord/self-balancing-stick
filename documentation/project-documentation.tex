...
A State Space model can be constructed from equations ?? and ??.  The
state space variables are omega, theta, and theta-dot.  The state space model
is then
x-dot = Am * x + B * u

Generally it is more convenient to monitor the relative velocity of the 
reaction wheel, wr = w - theta-dot.  The input vector is modified using
a transformation matrix in the following manner:
x-new = P*x
The state space model is then
x-dot-new = PAPinv *x-new + P*B*u
where Pinv is ----???---
