...
int Controller::get_PID_Voltage_Value() {
	float angle_Now, angle_Integral, angle_Speed_Now;
	if (angle == theta) {
		imu->get_theta_values(angle_Now, angle_Integral, angle_Speed_Now);
	} else {
		imu->get_omega_values(angle_Now, angle_Integral, angle_Speed_Now);
	}
